...
// 自動抓取拓元 Cookie 按鈕
const btnFetchTixcraftCookie = document.getElementById('btn_fetch_tixcraft_cookie');
if (btnFetchTixcraftCookie) {
    btnFetchTixcraftCookie.addEventListener('click', async () => {
        btnFetchTixcraftCookie.disabled = true;
        const originalText = btnFetchTixcraftCookie.textContent;
        btnFetchTixcraftCookie.textContent = '抓取中...';
        try {
            const response = await fetch('/fetch_cookie');
            const data = await response.json();
            if (data.success) {
                tixcraft_sid.value = data.cookie;
                tixcraft_sid.dispatchEvent(new Event('input'));
                btnFetchTixcraftCookie.textContent = '抓取成功！';
                setTimeout(() => { btnFetchTixcraftCookie.textContent = originalText; }, 2000);
            } else {
                alert('抓取失敗：' + data.message);
                btnFetchTixcraftCookie.textContent = originalText;
            }
        } catch (e) {
            alert('請求失敗：' + e);
            btnFetchTixcraftCookie.textContent = originalText;
        } finally {
            btnFetchTixcraftCookie.disabled = false;
        }
    });
}
